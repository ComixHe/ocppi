...
void printException(const std::shared_ptr<spdlog::logger> &logger,
                    std::string_view msg, std::exception_ptr ptr) noexcept try {
...
auto main() -> int {
  std::shared_ptr<spdlog::logger> logger = spdlog::stderr_color_mt("ocppi");
  logger->set_level(spdlog::level::trace);

  try {
    std::unique_ptr<ocppi::cli::CLI> cli;
    {
      auto crun = ocppi::cli::crun::Crun::New("/usr/bin/crun", logger);
      if (!crun) {
        printException(logger, "New crun object failed", crun.error());
        return -1;
      }
    ...
    auto bin = cli->bin();
    SPDLOG_LOGGER_DEBUG(logger, R"(Using OCI runtime CLI program "{}")",
                        bin.string());

    auto containerList = cli->list();
    if (!containerList) {
      printException(logger, "Run crun list failed", containerList.error());
      return -1;
    }
    ...
    for (auto container : containerList.value()) {
      ...
      SPDLOG_LOGGER_INFO(logger, "Existing container {}", containerJSON.dump());
    }

    auto state = cli->state(containerList->front().id);

    if (!state) {
      printException(logger, "Run crun state failed", state.error());
      return -1;
    }
    ...
    auto killResult =
        cli->kill(containerList->front().id, ocppi::runtime::Signal("SIGTERM"));

    if (!killResult) {
      printException(logger, "Run crun kill failed", killResult.error());
      return -1;
    }
