...
auto main() -> int {
  try {
    ...
    auto bin = cli->bin();
    std::cerr << "Using OCI runtime CLI program " << bin.string() << std::endl;
    ...
    for (auto container : containerList.value()) {
      ...
      std::cerr << "Existing container " << containerJSON.dump() << std::endl;
    }
    ...
    printException("Kill first crun container", std::current_exception());
    return -1;
  }
