...
class CommonCLI : public virtual CLI {
 protected:
#ifdef OCPPI_WITH_SPDLOG
  CommonCLI(std::filesystem::path, const std::shared_ptr<spdlog::logger> &);
  [[nodiscard]]
  ...
  [[nodiscard]]
  auto generateGlobalOptions(const runtime::GlobalOption &option) const noexcept
 ...
 public:
  ...
  auto state(const runtime::ContainerID &id) const noexcept
      -> tl::expected<runtime::state::types::State,
                      std::exception_ptr> override;
  [[nodiscard]]
  auto state(const runtime::ContainerID &id, const runtime::StateOption &option)
      const noexcept -> tl::expected<runtime::state::types::State,
                                     std::exception_ptr> override;
  ...
  [[nodiscard]]
  auto list() noexcept -> tl::expected<std::vector<types::ContainerListItem>,
                                       std::exception_ptr> override;
  [[nodiscard]]
